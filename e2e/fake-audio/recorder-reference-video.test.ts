...
test("stacks the mixer beside the reference video", async ({ page }) => {
  await createRecorderProject(page);

  await page.getByTestId("recorder-reference-video-button").click();
  await page.getByTestId("recorder-mixer-button").click();

  const video = page.getByTestId("recorder-youtube-reference");
  const mixer = page.getByTestId("recorder-mixer-panel");
  await expect(video).toBeVisible();
  await expect(mixer).toBeVisible();

  const initialVideoBox = await video.boundingBox();
  const initialMixerBox = await mixer.boundingBox();
  expect(initialVideoBox).not.toBeNull();
  expect(initialMixerBox).not.toBeNull();
  expect(initialMixerBox!.x + initialMixerBox!.width).toBeCloseTo(
    initialVideoBox!.x - 16,
    0,
  );
  expect(initialMixerBox!.y + initialMixerBox!.height).toBeCloseTo(
    initialVideoBox!.y + initialVideoBox!.height,
    0,
  );

  await dragBy(
    page,
    video.getByTestId("recorder-reference-video-resize-handle"),
    -80,
  );

  const resizedVideoBox = await video.boundingBox();
  const repositionedMixerBox = await mixer.boundingBox();
  expect(resizedVideoBox).not.toBeNull();
  expect(repositionedMixerBox).not.toBeNull();
  expect(repositionedMixerBox!.x + repositionedMixerBox!.width).toBeCloseTo(
    resizedVideoBox!.x - 16,
    0,
  );
});
