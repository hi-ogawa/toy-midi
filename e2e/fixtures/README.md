...
Raw f32le mono 22050 Hz PCM (the exact layout the Basic Pitch model consumes, accepted directly by `pnpm basic-pitch`) with C2/E2/G2 (MIDI 36/40/43) for 1s each, built from three harmonics plus a short attack/release envelope. In the expression, `st(0,…)` holds the current note's frequency and `st(1,…)` the per-note time driving the envelope:
