...
test("exports and imports a recorder project archive", async ({ page }) => {
  ...
  // Verify all audio uses clip arrays and the Capture destination survives export.
  ...
  expect(saved.audioTracks.map((track) => track.clips!.length).sort()).toEqual([
    1, 2,
  ]);
  expect(
    saved.audioTracks.find((track) => track.id === saved.armedTrackId)!.clips,
  ).toHaveLength(2);
  expect(
    saved.audioTracks.every(
      (track) => track.clip === undefined && track.timelineOffset === undefined,
    ),
  ).toBe(true);
...
test("imports a legacy recorder archive", async ({ page }) => {
  ...
  // Rename and save the migrated project before reopening and appending a take.
