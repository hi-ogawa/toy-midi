...
for (const kind of ["loop", "punch"] as const) {
  test(`${kind} menu creates, replaces, and clears its range`, async ({
    ...
    const rangeMenuButton = page.getByRole("button", {
    ...
    await rangeMenuButton.click();
    await expect(
      rangeMenu.getByRole("menuitem", { name: "Clear", exact: true }),
    ).toBeDisabled();
    await rangeMenu.getByRole("menuitem", { name: "New", exact: true }).click();
