...
test("undoes and redoes MIDI groups while cancelling drafts and clearing stale history", async ({
  ...
  const movedC4 = getMidiNote(row, { beat: 1, pitch: "C4" });
  const movedE4 = getMidiNote(row, { beat: 1.5, pitch: "E4" });
  ...
  await expect(getMidiNote(row, { beat: 1.5, pitch: "C4" })).toBeVisible();
  await page.keyboard.press("Control+z");
  await expect(c4).toBeVisible();
  await expect(e4).toBeVisible();
  await page.mouse.up();
  ...
  const g4 = await createMidiNote(page, row, { beat: 2, pitch: "G4" });
  await page.keyboard.press("Control+Shift+z");
  await expect(c4).toBeVisible();
  await expect(e4).toBeVisible();
...
test("undoes and redoes MIDI track creation, note edits, and deletion in order", async ({
  ...
  const first = await addMidiTrack(page);
  const note = await createMidiNote(page, first, {
    beat: 0.5,
    pitch: "C4",
  });
