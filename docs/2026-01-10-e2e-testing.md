...
```
e2e/
├── audio.spec.ts        # Audio file loading (1 test) - TO BE DELETED
...
**Current total: 32 tests → After consolidation: 31 tests**
...
**Phase 1: Debug API infrastructure**

- [ ] Expose `window.__store` in dev mode (in `project-store.ts`)
- [ ] Add TypeScript types for window extension
- [ ] Create `evaluateStore(page, fn)` helper in `e2e/helpers.ts`

**Phase 2: Test refactoring**

- [ ] Delete `audio.spec.ts`
- [ ] Refactor 12 tests per "Test Refactoring Plan" above
