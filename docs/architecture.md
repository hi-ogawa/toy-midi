...
The recorder's offline mix prototype snapshots imported clips and committed Capture regions into track-grouped source intervals in `recorder/mix.ts`. It shares track mute/solo/gain resolution with realtime playback, but schedules a separate stereo `OfflineAudioContext` at original speed without transport, metronome, reference video, or live input. Sources feed track gains and then master gain, leaving Capture and master processing boundaries for future shared effects. Rendering starts at timeline zero, crops negative placement, and retains muted tracks' extent. The renderer returns unclipped floats at the requested sample rate; the existing PCM WAV encoder owns hard clipping. `RecorderRuntime.renderMix()` supplies the current context sample rate. Export UI and download integration remain outside this prototype.
