...
- **System density and wrapping.** One system is one wrapped horizontal span containing the standard staff above the TAB staff. Initial screenshots were not directly comparable: the MuseScore reference was `1110x495` (`2.24:1`), while the OSMD capture was `842x420` (`2.00:1`) and therefore had substantially less horizontal score area. The viewer now defaults the actual score render width to `1110px` and exposes that width as a control, while preserving OSMD's natural measure spacing. Compare measures per system at matched score width before changing engraving density. Visible system count also depends on viewport dimensions, scale, and notation content.
