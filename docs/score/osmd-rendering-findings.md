...
- **System density and wrapping.** One system is one wrapped horizontal span containing the standard staff above the TAB staff. Initial screenshots were not directly comparable: the MuseScore reference was `1110x495` (`2.24:1`), while the OSMD capture was `842x420` (`2.00:1`) and therefore had substantially less horizontal score area. The viewer keeps OSMD's default measure-width factor `1.0` and exposes it as a validation control rather than hard-coding denser spacing. Compare measures per system only at matched content width; lower factors fit more measures but can reduce rhythmic readability. Visible system count also depends on viewport dimensions, scale, and notation content.
