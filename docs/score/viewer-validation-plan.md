...
- [ ] UI polish
  - Polish the Restart, Open, and Samples controls to follow the editor's existing button and icon conventions.
  - Right-align the loaded score name.
  - Replace `Open a Toy MIDI MusicXML export or load a generated sample.` with a shorter, actionable empty-state message.
