...
6. **Committed wasm artifact policy.** `src/lib/bass-pitch/pkg/` is a committed build artifact (~0.5 MB per generation) so the Cloudflare deploy needs no Rust toolchain, but nothing enforces that it matches `crates/`, and binary generations accumulate in history. Deferred decision between keeping it as is, adding a GitHub Actions check that rebuilds and diffs the pkg to catch drift, or switching to a CI-published artifact like demucs-onnx's pkg.pr.new package so no binary lives in git.
