...
2. Thread one per-chunk progress callback through the core (`run_pipeline` takes an `FnMut` receiving completed/total chunk counts), with both frontends as consumers, following the demucs-onnx pattern. The native CLI prints per-chunk progress lines, which both fixes its own silent multi-second runs and proves the chunked path plus callback plumbing where output is easiest to validate: the chunked CLI must reproduce the current unchunked CSVs on the fixtures before the wasm side changes. The wasm wrapper then only wraps the same callback as a `js_sys::Function` forwarded through the worker RPC exactly like the Basic Pitch `onProgress`, so the panel can show "Converting NN%".
