...
For a threshold $\theta_k$, trough $i$ qualifies when $z_i\lt\theta_k$. Rank the qualifying troughs by increasing lag, starting at rank 0. The rank $r_{i,k}$ can change with the threshold because the set of qualifying troughs changes.
