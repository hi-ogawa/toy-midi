...
6. **Committed wasm artifact policy.** Resolved by copying the demucs-onnx layout: no binary in git, the wasm builds from source in every environment, and the Cloudflare build bootstraps rustup inside plain `pnpm build` when `WORKERS_CI` is set. The committed `pkg/` generations from earlier remain in history but stop accumulating.
