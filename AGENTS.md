...
- `pnpm test-e2e-trace <test-file>` records DOM traces for visual review and generates `test-results/trace-pack.html`. Normal E2E runs leave tracing off. For selected traces on GitHub Actions with a PR artifact link, see [Manual E2E traces](docs/e2e-trace.md).
