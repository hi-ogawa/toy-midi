...
- `pnpm test-e2e-trace <test-file>` records DOM traces for visual review and generates `test-results/trace-pack.html`. Normal E2E runs leave tracing off. For selected branch traces on GitHub Actions with artifact links, see [E2E traces on GitHub Actions](docs/e2e-trace.md).
