...
function createTripletNotes() {
  // N and R occupy one triplet unit; N2 is an explicit two-unit note. Adjacent
  // rests are left as one gap because rests are generated by the exporter.
  const patterns = [
    "NNN",
    "NNR",
    "NRN",
  ...
  for (const [section, beatDuration] of [1, 2].entries()) {
    for (const [patternIndex, pattern] of patterns.entries()) {
      ...
      for (let index = 0; index < pattern.length; index++) {
        ...
        const units = pattern[index + 1] === "2" ? 2 : 1;
        if (type === "N") {
          notes.push(
            createNote({
              pitch:
