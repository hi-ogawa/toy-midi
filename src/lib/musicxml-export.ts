...
function buildMetricContext({
  ...
  const measureDuration =
    timeSignature.numerator * (4 / timeSignature.denominator) * DIVISIONS;
  const beatDuration =
    timeSignature.denominator === 8 && timeSignature.numerator % 3 === 0
      ? 1.5 * DIVISIONS
      : (4 / timeSignature.denominator) * DIVISIONS;
...
function isValidPlacement({
  ...
  if (candidate.triplet) {
    return (
      ...
      (candidate.duration <= DIVISIONS &&
        (cursor + candidate.duration) % DIVISIONS === 0)
    );
  }
