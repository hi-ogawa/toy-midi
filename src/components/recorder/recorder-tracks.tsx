...
export function TrackRow({
  ...
  // Keep enough room for the shared input controls on the Capture row.
  const minimumHeight = input ? 116 : 72;
  ...
  const resizeRef = usePointerDrag({
    ...
    onMove: (_event, { data: startHeight, deltaY }) => {
      onHeightChange(Math.max(minimumHeight, startHeight + deltaY));
    },
  });
  return (
    <div
